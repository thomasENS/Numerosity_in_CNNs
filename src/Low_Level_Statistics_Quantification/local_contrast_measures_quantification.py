...
from args import (mDir, Backgrounds, Objects, Versions, PS_Ranges,
                  Numerosity, Spacing, Size_Area, nPoints, Sigma)
...
def _Estimate_Weibull_Params(img_path, sigma):
    ...
    try:
        ...
        y, x  = np.histogram(edges, bins=256, density=True)  # compute density of contrasts
        dx = x[1:]-x[:-1]
...
for PS_range in PS_Ranges:
    ...
    for object_name in Objects:
        for bg_idx, bg_alpha in Backgrounds:
            ...
            Gamma_path = os.path.join(wDir, f'Sigma-{Sigma}_{object_name}_{bg_name}_Gamma.npy')
            Beta_path  = os.path.join(wDir, f'Sigma-{Sigma}_{object_name}_{bg_name}_Beta.npy')
            Area_path  = os.path.join(wDir, f'Sigma-{Sigma}_{object_name}_{bg_name}_Area.npy')

            if not os.path.isfile(Gamma_path): # Do not Estimate Params on Imgs that were already processed ! 

                Gamma_AVG, Beta_AVG, Area_AVG = np.zeros((nPoints,nPoints,nPoints)), np.zeros((nPoints,nPoints,nPoints)), np.zeros((nPoints,nPoints,nPoints))
                for N, ID, FD in ParkSpace_Description:
                    ...
                    for v_idx in Versions:
                        ...
                        img_path = os.path.join(iDir, object_name, f'Bg-{bg_idx}_Alpha{bg_alpha}', f'{object_name}-{N}_ID-{ID}_FD-{FD}_{bg_name}_version-{v_idx}.png')
                            
                        gamma_opt, beta_opt, area_error, bad_fit = _Estimate_Weibull_Params(img_path, Sigma)
                    ...
                    if len(Gamma) > 0: # If only bad_fit happened, zero will remain in the AVG arrays at the given coordinate.
