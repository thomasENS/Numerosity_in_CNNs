...
from args import (mDir, Backgrounds, Objects, Versions, PS_Ranges,
                  Numerosity, Spacing, Size_Area, SegMask)
    ...
    for bg_idx, bg_alpha in Backgrounds:
        for object_name in Objects:
            for v_idx in Versions:
                ...
                Mean_Lum_path = os.path.join(uDir, f'Mean_Lum{SegMask}_{object_name}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')
                Std_Lum_path  = os.path.join(uDir, f'Std_Lum{SegMask}_{object_name}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')

                IgM_mean, IgM_std = np.zeros((nPoints, nPoints, nPoints)), np.zeros((nPoints, nPoints, nPoints))
