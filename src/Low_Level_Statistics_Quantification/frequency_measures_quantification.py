...
from args import (mDir, PS_Ranges, Backgrounds, Objects, Versions, SegMask,
                  Numerosity, Spacing, Size_Area, ImgSize, cutoff_frequency)
...
for PS_range in PS_Ranges:
    ...
    sDir = os.path.join(sDir, 'Images') if SegMask == '' else os.path.join(sDir, 'Masks')
            ...
            for N, ID, FD in ParkSpace_Description:
                for v_idx in Versions:
                    ...
                    if os.path.isfile(img_path):
                        ...
                        result_path = os.path.join(uDir, f'rAvg_Fourier_Power{SegMask}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')
                        ...
                        if not os.path.isfile(result_path):
                            ...
                            If = fft.fftshift(fft.fft2(Ig)) # compute fft2 & shift the zero-frequency component to the center of the spectrum. 
...
for PS_range in PS_Ranges:
    ...
    Rotational_Averages = {(obj_name,f'bg-{bg_idx}_alpha{bg_alpha}'):{N:[] for N in Numerosity_Range[PS_range]} for bg_idx, bg_alpha in Backgrounds for obj_name in Objects}
                ...
                if N in Numerosity_Range[PS_range]:
                    for v_idx in Versions:
                        ...
                        if os.path.isfile(result_path):
                            ...
                            Rotational_Averages[object_name,f'bg-{bg_idx}_alpha{bg_alpha}'][N].append(rAvg.copy())
    ...
    for bg_idx, bg_alpha in Backgrounds:
        for object_name in Objects:
            ...
            nMax = np.min(
                (
                len(Rotational_Averages[object_name,f'bg-{bg_idx}_alpha{bg_alpha}'][Numerosity_Range[PS_range][0]]), 
                len(Rotational_Averages[object_name,f'bg-{bg_idx}_alpha{bg_alpha}'][Numerosity_Range[PS_range][-1]])
                )
            )
            Diff_rAVG_by_N = []
            for i in range(nMax):

                AVG_rAVG_small_N = Rotational_Averages[object_name,f'bg-{bg_idx}_alpha{bg_alpha}'][Numerosity_Range[PS_range][0]][i]
                AVG_rAVG_high_N  = Rotational_Averages[object_name,f'bg-{bg_idx}_alpha{bg_alpha}'][Numerosity_Range[PS_range][-1]][i]
    ...
    np.save(os.path.join(uDir, f'Difference_rAvg_Fourier_Power{SegMask}_Small_Vs_High_Numerosity.npy'), np.vstack(Difference_rAVG_by_N))
    np.save(os.path.join(uDir, f'Std_Diff_rAvg_Fourier_Power{SegMask}_Small_Vs_High_Numerosity.npy'), np.vstack(Std_Diff_rAvg_by_N))

fcycle_min_IA, fcycle_max_IA = {'subitizing':ImgSize/130, 'estimation':ImgSize/53},  {'subitizing':ImgSize/260, 'estimation':ImgSize/106}
dcycle_min_IA, dcycle_max_IA = {'subitizing':ImgSize/(2*130), 'estimation':ImgSize/(2*53)},  {'subitizing':ImgSize/(2*260), 'estimation':ImgSize/(2*106)}
...
for PS_range in PS_Ranges:
    ...
    Difference_rAVG = np.load(os.path.join(uDir, f'Difference_rAvg_Fourier_Power{SegMask}_Small_Vs_High_Numerosity.npy'))
    Std_Diff_rAvg   = np.load(os.path.join(uDir, f'Std_Diff_rAvg_Fourier_Power{SegMask}_Small_Vs_High_Numerosity.npy'))

    plt.figure()
    plt.title('[' + PS_range[0].upper() + PS_range[1:] + ' Range]\n' + 'Difference of Energy between high vs. low Numerosity')
    ...
    for i in range(len(Difference_rAVG)):
        plt.loglog(range(1,ImgSize+1), Difference_rAVG[i], alpha=.1, color='gray')
    plt.loglog(range(1,ImgSize+1), np.mean(Difference_rAVG, axis=0), 'k')
    plt.loglog([fcycle_min_IA[PS_range], fcycle_min_IA[PS_range]], [np.min(Difference_rAVG)*.9, np.max(Difference_rAVG)*1.1], 'g--', label='1 full cycle = smallest obj size')
    plt.loglog([fcycle_max_IA[PS_range], fcycle_max_IA[PS_range]], [np.min(Difference_rAVG)*.9, np.max(Difference_rAVG)*1.1], 'r--', label='1 full cycle = largest obj size')
    plt.loglog([dcycle_min_IA[PS_range], dcycle_min_IA[PS_range]], [np.min(Difference_rAVG)*.9, np.max(Difference_rAVG)*1.1], 'b--', label='1 demi cycle = smallest obj size')
    plt.loglog([dcycle_max_IA[PS_range], dcycle_max_IA[PS_range]], [np.min(Difference_rAVG)*.9, np.max(Difference_rAVG)*1.1], 'm--', label='1 demi cycle = largest obj size')
    plt.legend()
...
for PS_range in PS_Ranges:
    ...
    size_area, spacing, numerosity, nPoints = Size_Area[PS_range], Spacing[PS_range], Numerosity[PS_range], len(Numerosity[PS_range])
        ...
        for bg_idx, bg_alpha in Backgrounds:
            for v_idx in Versions:
                ...
                if not os.path.isfile(NRJ_High_SF_path):
                    ...
                    try:
                        Energy_Low_SF, Energy_High_SF = np.zeros([nPoints, nPoints, nPoints]), np.zeros([nPoints, nPoints, nPoints])
                        for N, ID, FD in ParkSpace_Description:
                            ...
                            idx_N, idx_Sp, idx_SzA = numerosity.index(N), spacing.index(Sp), size_area.index(SzA)          
                    
                            rAvg = np.load(os.path.join(uDir, f'rAvg_Fourier_Power{SegMask}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy'))
                            
                            Energy_Low_SF[idx_N, idx_Sp, idx_SzA]  = np.sum(rAvg[:cutoff_frequency[SegMask][PS_range]])
                            Energy_High_SF[idx_N, idx_Sp, idx_SzA] = np.sum(rAvg[cutoff_frequency[SegMask][PS_range]:])
...
def _get_theoritical_first_harmonic(segmask_path):
    ...
    Stimulus_SegMask = np.load(segmask_path) # values either 0 (background) or 1 (object)
...
for PS_range in PS_Ranges:
    ...
    for object_name in Objects:
        for bg_idx, bg_alpha in Backgrounds:
            ...
            base_stimulus_dir = os.path.join(object_name, f'Bg-{bg_idx}_Alpha{bg_alpha}')

            for v_idx in Versions:

                agg_mag_path = os.path.join(uDir, f'Aggregate_rAvg_Fourier_Magnitude{SegMask}_{object_name}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')
                ...
                if not os.path.isfile(agg_mag_path):
                    ...
                    for N, ID, FD in ParkSpace_Description:
                        ...
                        segmask_path = os.path.join(segDir, base_stimulus_dir, base_stimulus_path + '.npy')
                        if os.path.isfile(segmask_path):
                            ...
                            if SegMask == '':
                                ...
                                img_path = os.path.join(imgDir, base_stimulus_dir, base_stimulus_path + '.png')
                                I = io.imread(img_path)[:,:,:3] # load image and discard the alpha channal I[:,:,3] containing only 255s !
                                Ig = np.mean(I, axis=2) # convert to gray scale : (height, width, depth)
