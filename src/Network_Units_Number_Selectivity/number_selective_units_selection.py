...
for Model in Models:
    ...
    activated_units_cumsum = np.cumsum(overall_activity.astype(bool))
    ...
    (
        significance_units,
        non_significant_units,
        number_pvalues,
        significative_of_sets,
        sets_pvalues,
    ) = find_number_selective_features(activated_Features, alpha=0.01)
    ...
    non_significant_units_cumsum = np.cumsum(non_significant_units)
    nNonSignificantUnits = non_significant_units.sum()
    non_sigificant_units_idx = np.array(
        [
            np.where(non_significant_units_cumsum == idx)[0][0]
            for idx in range(1, nNonSignificantUnits + 1)
        ]
    )
    significative_of_sets_cumsum = np.cumsum(significative_of_sets)
    nSignificantofSetsUnits = significative_of_sets.sum()
    significative_of_sets_idx = np.array(
        [
            np.where(significative_of_sets_cumsum == idx)[0][0]
            for idx in range(1, nSignificantofSetsUnits + 1)
        ]
    )
    ...
    non_activated_units_idx = np.array(
        [idx for idx in range(nFeatures) if idx not in activated_units_idx]
    )
    activated_units_idx_pvalues = [
        (activated_units_idx[i], number_pvalues[i]) for i in range(nActivatedUnits)
    ]
    activated_units_idx_pvalues.sort(key=lambda x: x[1])
    ordered_units_idx_by_pvalues = np.concatenate(
        (
            np.array(
                [activated_units_idx_pvalues[i][0] for i in range(nActivatedUnits)]
            ),
            non_activated_units_idx,
        )
    )
    np.save(
        os.path.join(
    ...
    corrected_non_selective_units_idx = np.where(
        sets_pvalues[non_significant_units] * nFeatures < 0.05
    )[0]
    ...
    kept_units_idx = activated_units_idx[
        non_sigificant_units_idx[corrected_non_selective_units_idx]
    ]
    np.save(
        os.path.join(
            sDir,
    ...
    selective_units = [
        (number_pvalues[i], significance_units[i]) for i in range(len(number_pvalues))
    ]
    selective_units.sort(key=lambda x: x[0])
    significant_pvalues = (np.array(selective_units)[:, 0])[
        np.array(selective_units)[:, 1].astype(bool)
    ]
    np.save(
        os.path.join(sDir, f"{Model}_{Layer}_Anova_Pvalues_nImgs-{nImg}.npy"),
        significant_pvalues,
    )
