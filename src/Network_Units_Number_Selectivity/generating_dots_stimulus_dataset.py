...
def Stimulus_generation_Nasr(number_sets, image_iter):
    ...
    for ii in range(len(number_sets)):
        ...
        for jj in range(image_iter):
            ...
            circle_loc = np.ceil(circle_radius) + rd.randint(0, ImgSize - 2 * np.ceil(circle_radius)) + \
                        1j * (np.ceil(circle_radius) + rd.randint(0, ImgSize - 2 * np.ceil(circle_radius)))
        ...
        for jj in range(image_iter):
            ...
            while not (average_dist > 90 and average_dist < 100):
                ...
                while len(radtmp) < numtmp:
                    ...
                    loc = (np.ceil(rad) + rd.randint(0, ImgSize-2*np.ceil(rad))) + \
    ...
    for ii in range(len(number_sets)):
        ...
        for jj in range(image_iter):
            ...
            for thetaind in range(5):
                xtmpp[thetaind] = round(ImgSize / 2) + 110 * np.cos(theta + (thetaind - 1) * 2 * np.pi / 5)
                ytmpp[thetaind] = round(ImgSize / 2) + 110 * np.sin(theta + (thetaind - 1) * 2 * np.pi / 5)
            ...
            while len(radtmp) < numtmp:
                ...
                IN = inpolygon(np.real(circle_loc), np.imag(circle_loc), xtmpp[k], ytmpp[k])
                if circle_radius > 0:
            ...
            for kk in range(numtmp):
                ...
                if tmp > 0.75: # rectangle
                    imgtmpp = np.logical_and(np.abs(np.real(rtmp)) <= radtmp[kk], np.abs(np.imag(rtmp)) <= radtmp[kk])
    
                elif tmp > 0.5 and tmp < 0.75: # circle
                    imgtmpp = np.abs(rtmp) <= radtmp[kk]
                    
                elif tmp < 0.5 and tmp > 0.25: # ellipse
                    imgtmpp = (np.real(rtmp)**2 / (radtmp[kk]**2) + np.imag(rtmp)**2 / ((0.5*radtmp[kk])**2)) < 1
                    ...
                    for thetaind in range(3):
                        xtmp[thetaind] = np.real(loctmp[kk]) + radtmp[kk] * np.cos(theta + (thetaind-1) * 2 * np.pi / 3)
                        ytmp[thetaind] = np.imag(loctmp[kk]) + radtmp[kk] * np.sin(theta + (thetaind-1) * 2 * np.pi / 3)
                    ...
                    IN = inpolygon(xax.flatten(), yax.flatten(), xtmp[k], ytmp[k]).reshape(rax.shape)
                    imgtmpp = IN
...
image_sets_standard, image_sets_control1, image_sets_control2 = Stimulus_generation_Nasr(Nasr_Numerosity, nImg)

for i in range(nImg):
    for n in range(len(Nasr_Numerosity)):
        sPath = os.path.join(sDir, f'Standard_Set_N-{Nasr_Numerosity[n]}_Stim-{1+ i}.npy')
        np.save(sPath, image_sets_standard[:,:, i, n])
        sPath = os.path.join(sDir, f'Control-1_Set_N-{Nasr_Numerosity[n]}_Stim-{1+ i}.npy')
        np.save(sPath, image_sets_control1[:,:, i, n])
        sPath = os.path.join(sDir, f'Control-2_Set_N-{Nasr_Numerosity[n]}_Stim-{1+ i}.npy')
