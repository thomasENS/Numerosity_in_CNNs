...
def anova2(data, alpha):
    ...
    )  # average across the samples & factor B levels
    fB_mean = np.mean(
        data, axis=(0, 2), keepdims=True
    )  # average across the samples & factor A levels
...
def find_number_selective_features(features, alpha=0.01):
    ...
    (
        number_selective_features,
        number_non_selective_features,
        number_pvalues,
    ...
    for k in range(features.shape[3]):
        ...
        pNumber, sigNumber, pSets, sigSets, pInteraction, sigInteraction = anova2(
            features[:, :, :, k], alpha
        )
        number_selective_features.append(
            sigNumber and (not sigSets) and (not sigInteraction)
        )
        number_pvalues.append(pNumber)
        number_non_selective_features.append(
            (not sigNumber) and sigSets and (not sigInteraction)
        )
        sets_pvalues.append(pSets)
        significative_of_sets.append(sigSets)

    return (
        np.array(number_selective_features),
        np.array(number_non_selective_features),
        np.array(number_pvalues),
        np.array(significative_of_sets),
        np.array(sets_pvalues),
    )
