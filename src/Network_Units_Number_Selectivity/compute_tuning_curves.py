...
for Model in Models:
    ...
    Number_Selective_Units_Idx = np.load(os.path.join(sDir, f'{Model}_{Layer}_Nars_Number_Selective_Units_Reproduction_nImgs-{nImg}.npy'))
    ...
    Tuning_Curves = {N:np.zeros(nNumerosity) for N in Nasr_Numerosity}; n_Prefered_Numerosity = {}

    Prefered_Numerosity, Averaged_Response = {N:[] for N in Nasr_Numerosity}, {idx:[] for idx in Number_Selective_Units_Idx}
    ...
    for idx in Number_Selective_Units_Idx:
        ...
        prefered_numerosity_idx, activity_max, activity_min = np.argmax(avg_activations_by_numerosity), np.max(avg_activations_by_numerosity), np.min(avg_activations_by_numerosity)
        if activity_max != 0:
            Averaged_Response[idx] = ( (avg_activations_by_numerosity - activity_min) / activity_max).copy()
    ...
    np.save(os.path.join(sDir, f'{Model}_{Layer}_Nars_Number_Selective_Tuning_Curves_nImgs-{nImg}_normalised_between_0_1.npy'), tuned_curves)