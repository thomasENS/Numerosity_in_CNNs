...
def beatiful_violin_plot(y_data_S, y_data_E, jitter, title, ft_text, COLOR_SCALE_S, COLOR_SCALE_E, ylabel=None, Labels=None, HLINES=None, width=.3, ax=None):
    '''
    ...
    if HLINES is not None:
        for h in HLINES:
            ax.axhline(h, color='k', ls=(0, (3, 5)), alpha=.8, zorder=0)
...
def averaging_diff_per_obj_bg(d):
    ...
    '''
    avg_bg  = [np.mean(d[i*nBackgrounds:(i+1)*nBackgrounds]) for i in range(nBackgrounds)]
    avg_obj = []
