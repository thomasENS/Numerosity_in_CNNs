...
## Axes limits
lims = [-1.5060440607531775 + 0.2, 0.6683987855562563]
Ticks = np.log(np.array([0.22, Chance_Level[Modality], 0.8, 1.5]))
...
legend_chance = [
    Patch(facecolor="gray", alpha=0.7, edgecolor="k", label="MAE Better\nthan Chance")
]

# %% Fig 2C : Coarse Grained Generalisation
MAE, STD = {PS_range: {Model: [] for Model in Models} for PS_range in PS_Ranges}, {
    PS_range: {Model: [] for Model in Models} for PS_range in PS_Ranges
}
for PS_range in PS_Ranges:
    uDir = os.path.join(result_dir, f"PS_{PS_range[0].upper() + PS_range[1:]}_Range")

    for Model in Models:
        MAE[PS_range][Model] = np.load(
            os.path.join(
                uDir,
                f"Full_Generalisation_{Model}_Decoding_Log_{Modality}_Across_Hierarchy_Score_MAE.npy",
            )
        )
        STD[PS_range][Model] = np.load(
            os.path.join(
                uDir,
                f"Full_Generalisation_{Model}_Decoding_Log_{Modality}_Across_Hierarchy_Score_Std.npy",
            )
        )
...
for case in Cases:
    ...
    axes[n - 1].fill_between(
        x=[np.log(0.1), np.log(Chance_Level[Modality])],
        y1=np.log(0.1),
    ...
    axes[n - 1].fill_between(
        x=[np.log(Chance_Level[Modality]), np.log(2.3)],
        y1=np.log(0.1),
        y2=np.log(Chance_Level[Modality]),
        color="gray",
        alpha=0.2,
    ...
    for Model in cModels[case]:
        for i in range(5):
            x, y = np.log(MAE["estimation"][Model][i]), np.log(
...
lgd_models = axes[0].legend(
    handles=legend_models, loc="upper right", bbox_to_anchor=(1, 1)
)
...
for n in range(2):
    axes[n].plot(
        [np.log(0.1), np.log(Chance_Level[Modality])],
    ...
    axes[n].plot(
        [np.log(Chance_Level[Modality]), np.log(2)],
        [np.log(Chance_Level[Modality])] * 2,
        "--",
        color="gray",
        linewidth=1.0,
...
plt.savefig(
    os.path.join(
        saving_dir, "Coarse_Grained_Numerosity_Decoding_Classical_Generalisation.svg"
    ),
    dpi=300,
    bbox_inches="tight",
)
...
for PS_range in PS_Ranges:
    ...
    for Model in Models:
        MAE[PS_range][Model] = np.load(
            os.path.join(
                uDir,
                f"Full_Generalisation_Controlled_Non_Numerical_Params_{Model}_across_Hierarchy_Decoding_Log_{Modality}_Score_MAE.npy",
            )
        )
        STD[PS_range][Model] = np.load(
            os.path.join(
                uDir,
                f"Full_Generalisation_Controlled_Non_Numerical_Params_{Model}_across_Hierarchy_Decoding_Log_{Modality}_Score_Std.npy",
            )
        )
...
plt.savefig(
    os.path.join(
        saving_dir,
        "Coarse_Grained_Numerosity_Decoding_Congruent_Incongruent_Generalisation.svg",
    ),
    dpi=300,
    bbox_inches="tight",
)
