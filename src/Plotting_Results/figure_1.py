...
COLOR_SCALE_Legend = [
    Patch(facecolor="#3CAB88", edgecolor="k", label="Numerosity"),
...
legend = [
    Patch(facecolor="#CECECE", edgecolor="k", label="Subitizing Range"),
    Patch(facecolor="#707070", edgecolor="k", label="Estimation Range"),
]

Height = 6
nrows, ncols = 3, 2
fig, axs = plt.subplots(
    nrows=nrows, ncols=ncols, figsize=(ncols * Height, nrows * Height)
)
...
for i in range(nrows):
    for j in range(ncols):
        ...
        k = ncols * i + j
        title = Title[k]
        Diff_S, Dist_Obj_S, Dist_Bg_S, Diff_E, Dist_Obj_E, Dist_Bg_E = _load_data(
            dirBase[k], DistPath[k], DiffPath[k]
        )
        if k == 5:
...
axs[0, 0].legend(
    ...
    fontsize=ft_legend,
)  # title='Numerosity Range', title_fontsize=ft_legend, ncol=1, columnspacing=.8)
axs[0, 1].legend(
    handles=COLOR_SCALE_Legend,
    loc="lower right",
    bbox_to_anchor=(1.03, 1.08),
    fontsize=ft_legend,
    title="Changes across ...",
    title_fontsize=ft_legend,
    ncol=3,
    columnspacing=0.8,
)
for i in range(nrows):
...
plt.savefig(
    os.path.join(
        saving_dir,
        "Low_Level_Statistics_Influence_by_Numerosity_Backgrounds_Objects.svg",
    ),
    bbox_inches="tight",
    dpi=300,
)
