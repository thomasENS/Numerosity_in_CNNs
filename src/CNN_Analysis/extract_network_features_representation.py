...
preprocess = ImageNetStimuliPreProcessing()

for Model in Models:
    for Mode in Training_Modes:

        FeaturesExtractor = TorchHubFeatureExtractor(Model, Mode)
        ...
        for PS_range in PS_Ranges:
            ...
            iDir = os.path.join(
                ...
                f"PS_{PS_range[0].upper() + PS_range[1:]}_Range",
                "Images",
            )
            fDir = os.path.join(
                rDir,
                        ...
                        for v_idx in Versions:
                            ...
                            features_paths = [
                                os.path.join(
                                    ...
                                    f"{Model_Names[Mode][Model]}_{layer}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy",
                                )
                                for layer in Layers
                            ]
                            is_not_files = [
                            ...
                            if np.any(
                                ...
                                if os.path.isfile(
                                    img_path
                                ):  # Do not try to compute representation of an image that does not exist !
                        ...
                        for v_idx in Versions:
                            ...
                            features_paths = [
                                os.path.join(
                                    ...
                                    object_name,
                                    f"Bg-{bg_idx}_Alpha{bg_alpha}",
                                    f"{Model_Names[Mode][Model]}_{layer}_Mask_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy",
                                )
                                for layer in Layers
                            ]
                            is_not_files = [
                            ...
                            if np.any(
                                ...
                                img_path = os.path.join(
                                    ...
                                    object_name,
                                    f"Bg-{bg_idx}_Alpha{bg_alpha}",
                                    f"{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy",
                                )
...
preprocess = {"": ImageNetStimuliPreProcessing(), "_Mask": mask_dataset_preprocessing}
...
for PS_range in PS_Ranges:
    for UseSegMask in Stimulus_Types:
        ...
        iDir = (
            os.path.join(uDir, "Masks")
            if UseSegMask == "_Mask"
            else os.path.join(uDir, "Images")
        )
        fDir = os.path.join(
            rDir,
            "Photorealistic_Dataset",
                ...
                for N, ID, FD in ParkSpace_Description:
                    for v_idx in Versions:
                        ...
                        img_pixels_path = os.path.join(
                            ...
                            object_name,
                            f"Bg-{bg_idx}_Alpha{bg_alpha}",
                            f"RawPixels{UseSegMask}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy",
                        )
        ...
        for n in Nasr_Numerosity:
            for Set_Name in ["Standard", "Control-1", "Control-2"]:
                ...
                features_path = os.path.join(
                    ...
                    Model_Names[Mode][Model],
                    Layers[-1],
                    f"{Model_Names[Mode][Model]}_{Layers[-1]}_Set_Name_Set_N-{n}_Stim-{i+1}.npy",
                )
                if not os.path.isfile(
                    features_path
                ):  # Do not compute a representation that was already created !
                    ...
                    img_path = os.path.join(
                        iDir, f"{Set_Name}_Set_N-{n}_Stim-{i+1}.npy"
                    )
                    ...
                    if os.path.isfile(
                        ...
                        for layer in Layers:
                            features_path = os.path.join(
                                ...
                                Model_Names[Mode][Model],
                                layer,
                                f"{Model_Names[Mode][Model]}_{layer}_Set_Name_Set_N-{n}_Stim-{i+1}.npy",
                            )
                            np.save(features_path, features[layer])
