...
from args import (mDir, dDir, Naturals, Animals, PS_Ranges, nSampled, nCVloop,
                  Alphas, Training_Modes, Models, Layers, Model_Names,
                  SubSpace, subSpaceTrain, subSpaceTest, Modalities,
                  Stimulus_Types)
...
def _load_stimuli_dataset(model, layer, ps_range, modality, load_space_idx, selected_features_space_idx, sampling_matrix, v_idx=1, _mask='', target_scale='Log'):
    '''
    ...
    assert layer in ['Conv1', 'Conv2', 'Conv3', 'Conv4', 'Conv5'], 'AlexNet layers are ConvX with X in {1 ... 5}'
            ...
            if sampling_matrix[i,j]:
                ...
                for N, ID, FD in ParkSpace_Description:
                    ...
                    if os.path.isfile(features_path):
                        try: 
                            X.append(_load_features(features_path, model, layer, modality, selected_features_space_idx))
...
def _ridge_decoding_across_hierarchy_JOBLIB(PS_range, Model, Modality, Layers, subSpaceTrain, subSpaceTest, UseSegMask='', target_scale='Log'):

    clf = RidgeCV(alphas=Alphas)

    results_path = os.path.join(lDir, f'Full_Generalisation{UseSegMask}_{Model}_Decoding_{target_scale}_{Modality}')
    ...
    for i in range(len(Layers)):
        ...
        for sTrain in subSpaceTrain:
            ...
            for k in range(nCVloop):
                ...
                X_train, y_train = _load_stimuli_dataset(Model, Layer, PS_range, Modality, sTrain, sTrain, Sampling_Matrix, _mask=UseSegMask, target_scale=target_scale)
                ...
                for sTest in subSpaceTest[sTrain]:
                    X_test, y_test = _load_stimuli_dataset(Model, Layer, PS_range, Modality, sTest, sTrain, Sampling_Matrix, _mask=UseSegMask, target_scale=target_scale)
    ...
    for Model_Name in Models:
        for Mode in Training_Modes:
            ...
            for UseSegMask in Stimulus_Types:
                ...
                Parallel(n_jobs=60)(delayed(_ridge_decoding_across_hierarchy_JOBLIB)(
                    PS_range,
                    Model,
                    Modality,
                    Layers,
                    ...
                    target_scale=TargetScale) for Modality in Modalities
                )