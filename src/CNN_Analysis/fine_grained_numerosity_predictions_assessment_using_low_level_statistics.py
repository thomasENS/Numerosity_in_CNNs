...
def _load_stimuli_statistics(
    ...
    assert UseSegMask in [
        "",
        "_Mask",
    ], "UseSegMask as '' for use of the PhotoRealistic Stimuli & as '_Mask' for associated Segmentation Masks"
        ...
        for Mode in Training_Modes:
            ...
            for Layer in Layers:
                ...
                results_path = (
                    ...
                    if Model != "RawPixels"
                    else os.path.join(sDir, f"FGG_OLS_{Model}_Decoding_Log_{Modality}")
                )
                predictions_path = (
                    os.path.join(
                        sDir,
                        f"Finer_Grain_Generalisation{UseSegMask}_{Model}_{Layer}_Decoding_Log_{Modality}",
                    )
                    if Model != "RawPixels"
                    else os.path.join(
                        sDir,
                        f"Finer_Grain_Generalisation{UseSegMask}_{Model}_Decoding_Log_{Modality}",
                    )
                )
                save_path = (
                    results_path
                    + f"_Across_Objects_LLS_Explained_Variance{UseSegMask}.npy"
                )
                ...
                for Layer in Layers:
                    ...
                    results_path = (
                        ...
                        if Model != "RawPixels"
                            ...
                            sDir, f"FGG_OLS_{Model}_Decoding_Log_{Modality}"
                        )
                    )
                    predictions_path = (
                        os.path.join(
                    ...
                    save_path = (
                        results_path
                        + f"_Across_Backgrounds_LLS_Explained_Variance{UseSegMask}.npy"
                    )
