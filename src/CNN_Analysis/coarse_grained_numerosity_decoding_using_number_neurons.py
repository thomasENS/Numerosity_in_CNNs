...
from args import (mDir, dDir, Naturals, Animals, PS_Ranges, nSampled, nCVloop,
                  Alphas, SubSpace, subSpaceTrain, subSpaceTest, nFeatures,
                  Percentages, nPercentages, nImg)
...
def _load_stimuli_dataset(model, layer, load_space_idx, number_selective_units, sampling_matrix, numerosity, v_idx=1, _mask=''):
    '''
            ...
            for j in range(nBackgrounds):
                ...
                if sampling_matrix[i,j]:
                    ...
                    for N, ID, FD in ParkSpace_Description:
                        if N in numerosity:
                            if model != 'RawPixels':
                                features_path = os.path.join(uDir, model, layer, object_name, f'Bg-{bg_idx}_Alpha{bg_alpha}', f'{model}_{layer}{_mask}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')
                            else:
                                features_path = os.path.join(uDir, 'RawPixels', object_name, f'Bg-{bg_idx}_Alpha{bg_alpha}', f'RawPixels{_mask}_{object_name}-{N}_ID-{ID}_FD-{FD}_bg-{bg_idx}_alpha{bg_alpha}_version-{v_idx}.npy')

                            if os.path.isfile(features_path):
                                if model != 'RawPixels':
                                    X.append(np.load(features_path)[number_selective_units])
                                else:
...
for Model in Models:
    ...
    Number_Selective_Features_Idx = np.load(os.path.join(sDir, f'{Model}_{Layer}_Nars_Number_Selective_Units{Correction}_Reproduction_nImgs-{nImg}.npy'))
    Non_Selective_Population_Idx  = np.array([idx for idx in range(nFeatures) if idx not in Number_Selective_Features_Idx])
    Non_Selective_Features_Idx    = np.load(os.path.join(sDir, f'{Model}_{Layer}_Nars_Number_Non_Selective_Units{Correction}_Reproduction_nImgs-{nImg}.npy'))

    results_path = os.path.join(sDir, f'Full_Generalisation{UseSegMask}_{Model}_{Layer}_Decoding_Log_N_from_Number_Selective_Units{Correction}_nImgs-{nImg}')
    ...
    for sTrain in subSpaceTrain:
        ...
        for k in range(nCVloop):
            ...
            X_train, y_train = _load_stimuli_dataset(Model, Layer, sTrain, Number_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
            ...
            for sTest in subSpaceTest[sTrain]:
                X_test, y_test = _load_stimuli_dataset(Model, Layer, sTest, Number_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
    ...
    for sTrain in subSpaceTrain:
        ...
        for k in range(nCVloop):
            ...
            X_train, y_train = _load_stimuli_dataset(Model, Layer, sTrain, Non_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
            ...
            ## Compute Outer Loop Ridge Regression MAE score
            for sTest in subSpaceTest[sTrain]:
                X_test, y_test = _load_stimuli_dataset(Model, Layer, sTest, Non_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
                MAE_InterCV.append(MAE(clf.predict(X_test), y_test))
    ...
    for sTrain in subSpaceTrain:
        ...
        for k in range(nCVloop):
            ...
            X_train, y_train = _load_stimuli_dataset(Model, Layer, sTrain, Non_Selective_Population_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
            ...
            ## Compute Outer Loop Ridge Regression MAE score
            for sTest in subSpaceTest[sTrain]:
                X_test, y_test = _load_stimuli_dataset(Model, Layer, sTest, Non_Selective_Population_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
                MAE_InterCV.append(MAE(clf.predict(X_test), y_test))
    ...
    for sTrain in subSpaceTrain:
        ...
        for k in range(nCVloop):
            ...
            X_train, y_train = _load_stimuli_dataset('RawPixels', Layer, sTrain, Non_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
            ...
            ## Compute Outer Loop Ridge Regression MAE score
            for sTest in subSpaceTest[sTrain]:
                X_test, y_test = _load_stimuli_dataset('RawPixels', Layer, sTest, Non_Selective_Features_Idx, Sampling_Matrix, Numerosity, _mask=UseSegMask)
                MAE_InterCV.append(MAE(clf.predict(X_test), y_test))
...
for Model in Models:
    ...
    results_path = os.path.join(sDir, f'Coarse_Grained_Generalisation_{Model}_{Layer}_Decoding_Log_N_Redundancy_Assessement_Photorealistic_Stimuli')
    score_MAE_RandomUnits_CV, score_Std_RandomUnits_CV = np.zeros(nPercentages), np.zeros(nPercentages)
    ...
    for sTrain in subSpaceTrain:
        ...
        X_train, y_train = _load_stimuli_dataset(Model, Layer, sTrain, Neurons_Idx, Full_Sampling_Matrix, Numerosity)
        X_test, y_test   = _load_stimuli_dataset(Model, Layer, subSpaceTest[sTrain], Neurons_Idx, Full_Sampling_Matrix, Numerosity)
    ...
    np.save(results_path + '_Score_MAE_Randomly_Chosen_Units.npy', score_MAE_RandomUnits_CV)
    np.save(results_path + '_Score_Std_Randomly_Chosen_Units.npy', score_Std_RandomUnits_CV)