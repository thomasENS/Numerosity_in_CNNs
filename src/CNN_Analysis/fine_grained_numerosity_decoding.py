...
from args import (mDir, dDir, Backgrounds, Objects, PS_Ranges, Versions, nVersions, Alphas,
...
def _load_stimuli_object(iDir, fDir, model, layer, ps_range, modality, object_name, selected_features_space_obj, v_idx=1, _mask='', target_scale='Log'):
    '''
    ...
    for bg_idx, bg_alpha in Backgrounds:
        ...
        for N, ID, FD in ParkSpace_Description:
            ...
            if os.path.isfile(features_path): 
                X.append(_load_features(iDir, features_path, model, layer, modality, selected_features_space_obj))
                y.append(_load_labels(N, ID, FD, modality, target_scale))

    return np.array(X), np.array(y) - np.array(y).mean() # centering target

def _load_stimuli_background(iDir, fDir, model, layer, ps_range, modality, bg_name, selected_features_space_bg, v_idx=1, _mask='', target_scale='Log'):
    '''
    ...
    bg_idx, bg_alpha = bg_name.split('_')[0].split('bg-')[1], bg_name.split('_')[1].split('alpha')[1]
    ...
    for object_name in Objects:
        ...
        for N, ID, FD in ParkSpace_Description:
            ...
            if os.path.isfile(features_path): 
                X.append(_load_features(iDir, features_path, model, layer, modality, selected_features_space_bg))
                y.append(_load_labels(N, ID, FD, modality, target_scale))
...
def _ridge_finer_grain_generalisation_JOBLIB(Model, Layer, PS_range, UseSegMask='', target_scale='Log'):
    ...
    results_path = os.path.join(lDir, f'Finer_Grain_Generalisation{UseSegMask}_{Model}_{Layer}_Decoding_{target_scale}_{Modality}') if Model != 'RawPixels' else os.path.join(lDir, f'Finer_Grain_Generalisation_{Model}_Decoding_{target_scale}_{Modality}')

    score_MAE, score_Std = np.zeros([nObjects, nObjects]), np.zeros([nObjects, nObjects])
    predictions = np.zeros((nObjects, nObjects, nBackgrounds * 64 * nVersions)) # shape : (20, 20, 2560) i.e. pred[i, j, :1280] correspond to the predictions of the 2nd version of the stimuli (since X_test on 3-v_idx !)
    ...
    if not os.path.isfile(results_path + '_Across_Objects' + '_Score_MAE.npy'):
        ...
        for i in range(nObjects):
            ...
            y_pred, MAE_InterCV = {j:[] for j in range(nObjects)}, {j:[] for j in range(nObjects)}
            for v_idx in Versions:

                X_train, y_train = _load_stimuli_object(iDir, fDir, Model, Layer, PS_range, Modality, objTrain, objTrain, v_idx, _mask=UseSegMask, target_scale=target_scale)
                ...
                for j in range(nObjects):
                    ...
                    X_test, y_test = _load_stimuli_object(iDir, fDir, Model, Layer, PS_range, Modality, objTest, objTrain, 3 - v_idx, _mask=UseSegMask, target_scale=target_scale) ## y=3-x i.e. (x=1,y=2 & x=2,y=1)
    ...
    score_MAE, score_Std = np.zeros([nBackgrounds, nBackgrounds]), np.zeros([nBackgrounds, nBackgrounds])
    predictions = np.zeros((nBackgrounds, nObjects, nBackgrounds * 64 * nVersions)) # shape : (20, 20, 2560) i.e. pred[i, j, :1280] correspond to the predictions of the 2nd version of the stimuli (since X_test on 3-v_idx !)
    ...
    if not os.path.isfile(results_path + '_Across_Backgrounds' + '_Score_MAE.npy'):
        ...
        for i in range(nBackgrounds):
            ...
            y_pred, MAE_InterCV = {j:[] for j in range(nBackgrounds)}, {j:[] for j in range(nBackgrounds)}
            for v_idx in Versions:

                X_train, y_train = _load_stimuli_background(iDir, fDir, Model, Layer, PS_range, Modality, bgTrain, bgTrain, v_idx, _mask=UseSegMask, target_scale=target_scale)
                ...
                for j in range(nBackgrounds):
                    ...
                    X_test, y_test = _load_stimuli_background(iDir, fDir, Model, Layer, PS_range, Modality, bgTest, bgTrain, 3 - v_idx, _mask=UseSegMask, target_scale=target_scale)
...
for Model_Name in Models:
    for Mode in Training_Modes:
        ...
        for UseSegMask in Stimulus_Types:
            ...
            Parallel(n_jobs=-1)(delayed(_ridge_finer_grain_generalisation_JOBLIB)(
                Model,
                Layer,
                PS_range,
                UseSegMask=UseSegMask,
                target_scale=TargetScale) for Layer in Layers for PS_range in PS_Ranges
            )
