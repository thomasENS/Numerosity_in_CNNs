...
def _load_stimuli_labels(
    model, layer, ps_range, modality, load_space_idx, v_idx=1, _mask=""
):
...
def _load_stimuli_object_dataset(
    model, layer, ps_range, modality, object_name, _mask=""
):
    """
...
def _load_stimuli_background_dataset(
    model, layer, ps_range, modality, bg_name, _mask=""
):
    """
                ...
                for Layer in Layers:
                    ...
                    for sub_space_idx in [1, 2, 3, 4]:
                        ...
                        if not os.path.isfile(
                            os.path.join(
                                iDir,
                                f"{Model_Names[Mode][Model]}_{Layer}{UseSegMask}_subSpace-{sub_space_idx}_N_selected_features_idx.npy",
                            )
                        ):  ## Do not compute Features Idx that were already computed

                            print(
                                Model_Names[Mode][Model], Layer, PS_range, sub_space_idx
                            )
                            X = _load_stimuli_features(
                            ...
                            for modality in Modalities:
                                ...
                                idx_path = os.path.join(
                                    iDir,
                                    f"{Model_Names[Mode][Model]}_{Layer}{UseSegMask}_subSpace-{sub_space_idx}_{modality}_selected_features_idx.npy",
                                )
                                ...
                                if not os.path.isfile(
                                    ...
                                    sorted_pValue_Idx = np.argsort(
                                        -f_stat
                                    )  # NaN values are put at the end, so, not causing any issue as long as there is more non-NaN than nFeatures
                                    selected_features_Idx = sorted_pValue_Idx[
                                        :nFeatures
                                    ]

                                    if Model == "alexnet" and Layer in [
                                        "Conv4",
                                        "Conv5",
                                    ]:
                                        ...
                                        if len(p_value) - nNaN < nFeatures:
                                            print(
                                                ...
                                                sub_space_idx,
                                                modality,
                                                f"Careful, {nNaN} nNaN values relative to {len(p_value) - nFeatures} Features.",
                                            )
                                        else:
                                            np.save(idx_path, selected_features_Idx)
                ...
                for Mode in Training_Modes:
                    ...
                    for Layer in Layers:
                        ...
                        for bg_idx, bg_alpha in Artificials + Naturals:
                            ...
                            idx_path = os.path.join(
                                iDir,
                                f"{Model_Names[Mode][Model]}_{Layer}{UseSegMask}_subSpace-{bg_name}_{Modality}_selected_features_idx.npy",
                            )
                            ...
                            if not os.path.isfile(
                                ...
                                print(
                                    Model_Names[Mode][Model], Layer, PS_range, bg_name
                                )

                                X, y = _load_stimuli_background_dataset(
                                    Model_Names[Mode][Model],
                                    Layer,
                                    PS_range,
                                    Modality,
                        ...
                        for obj_name in Animals + Tools:
                            ...
                            idx_path = os.path.join(
                                iDir,
                                f"{Model_Names[Mode][Model]}_{Layer}{UseSegMask}_subSpace-{obj_name}_{Modality}_selected_features_idx.npy",
                            )
                            ...
                            if not os.path.isfile(
                                ...
                                print(
                                    Model_Names[Mode][Model], Layer, PS_range, obj_name
                                )

                                X, y = _load_stimuli_object_dataset(
                                    Model_Names[Mode][Model],
                                    Layer,
                                    PS_range,
                                    Modality,
