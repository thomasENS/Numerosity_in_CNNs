...
from args import (mDir, dDir, Backgrounds, Objects, PS_Ranges, Versions, Alphas,
...
def _load_stimuli(ps_range, modality, statistics, v_idx, object_name=None, bg_name=None, dT=None, dC=None, UseSegMask=''):
    ...
    assert len(X) == len(y), f'Mismatch in the number of samples between Features : {len(X)} & Targets : {len(y)}'
...
for PS_range in PS_Ranges:
    ...
    for Statistics in [[stat] for stat in Low_Level_Statistics]+ [Low_Level_Statistics]:
        ...
        results_path = os.path.join(sDir, f'UFG_Decoding_Log_{Modality}_Using_{"_".join(Statistics)}_Statistics')
        ...
        if not os.path.isfile(results_path + '_Across_Objects' + '_Score_MAE.npy'):
            ...
            for i in range(nObjects):
                ...
                for v_idx in Versions:
                    ...
                    X_train, y_train = _load_stimuli(PS_range, Modality, Statistics, v_idx, object_name=objTrain, bg_name=None, dT=dT, dC=dC)
                    ...
                    for j in range(nObjects):
                        ...
                        X_test, y_test = _load_stimuli(PS_range, Modality, Statistics, 3 - v_idx, object_name=objTest, bg_name=None, dT=dT, dC=dC) ## y=3-x i.e. (x=1,y=2 & x=2,y=1)
        ...
        if not os.path.isfile(results_path + '_Across_Backgrounds' + '_Score_MAE.npy'):
            ...
            for i in range(nBackgrounds):
                ...
                for v_idx in Versions:
                    ...
                    X_train, y_train = _load_stimuli(PS_range, Modality, Statistics, v_idx, object_name=None, bg_name=bgTrain, dT=dT, dC=dC)
                    ...
                    for j in range(nBackgrounds):
                        ...
                        X_test, y_test = _load_stimuli(PS_range, Modality, Statistics, 3 - v_idx, object_name=None, bg_name=bgTest, dT=dT, dC=dC)
