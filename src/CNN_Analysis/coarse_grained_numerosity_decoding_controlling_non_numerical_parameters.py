...
Orange_Set = {
    "estimation": [(6, 10.7, 8.9), (10, 11.0, 8.7), (15, 11.1, 8.5), (24, 11.3, 8.3)],
    "subitizing": [(1, 11.5, 9.7), (2, 11.8, 9.5), (3, 12.0, 9.4), (4, 12.1, 9.1)],
}
...
def _load_controlled_non_numerical_parameters_stimuli_dataset(
    model,
    layer,
    ps_range,
    load_space_idx,
    selected_features_space_idx,
    controlling_condition_set,
    _mask="",
    target_scale="Log",
):
    """
...
for PS_range in PS_Ranges:
    ...
    Controlling_Set = {0: Yellow_Set[PS_range], 1: Orange_Set[PS_range]}
    Control_Condition = [0, 1]
        ...
        for Model_Name in Models:
            for Mode in Training_Modes:
                ...
                results_path = os.path.join(
                    lDir,
                    f"Full_Generalisation_Controlled_Non_Numerical_Params{UseSegMask}_{Model}_across_Hierarchy_Decoding_{TargetScale}_N",
                )
                    ...
                    for i in range(nLayers):
                        ...
                        for control in Control_Condition:
                            ...
                            for sTrain in subSpaceTrain:
                                ...
                                X_train, y_train = (
                                    _load_controlled_non_numerical_parameters_stimuli_dataset(
                                        Model,
                                        Layers[i],
                                        PS_range,
                                        sTrain,
                                        sTrain,
                                        Controlling_Set[control],
                                        _mask=UseSegMask,
