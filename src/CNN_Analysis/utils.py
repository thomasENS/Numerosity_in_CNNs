...
def dot_dataset_preprocessing(img_path):
    ...
    img_tensor = imageNet_Normalisation(
        img_tensor
    )  # Normalisation applied to tensor whose values in [0,1]
    return img_tensor.unsqueeze(
        0
    )  # imageNet pretrained Nets takes (nBatch, 3, H, W) as inputs
...
class ImageNetStimuliPreProcessing(nn.Module):

    def __init__(self):
        super(ImageNetStimuliPreProcessing, self).__init__()
    ...
    def _load_stimuli(self, path):
        ...
        numpy_image = np.array(pil_image)[
            :, :, :3
        ]  # discard the Alpha informationless dimension
...
class LayersFeatureExtractor(nn.Module):
    """
    ...
    def __init__(self, model, layers):
        super(LayersFeatureExtractor, self).__init__()
        self.model = model
        self.model.eval()
...
class HCNNFeatureExtractor(LayersFeatureExtractor):
    """
    ...
    def __init__(self, model, layers, layers_name):
        ...
        assert len(layers) == len(
            layers_name
        ), f"Mismatch of length between the number of selected layers : {len(layers)} and their associated user-friendly name :  {len(layers_name)} given."

        self.Model_RENAMED_LAYERS = {
            layers[i]: layers_name[i] for i in range(len(layers))
        }
        Model_CONVLAYER_NAME = {layers_name[i]: layers[i] for i in range(len(layers))}
        model_layers = [
            Model_CONVLAYER_NAME[layer_name]
            for layer_name in layers_name[: len(layers)]
        ]

        super(HCNNFeatureExtractor, self).__init__(model, model_layers)

        self._renamed_features = {
            layer_name: torch.empty(0) for layer_name in layers_name[: len(layers)]
        }

    def forward(self, x):
        features = LayersFeatureExtractor.forward(self, x)
        for key in features.keys():
            self._renamed_features[self.Model_RENAMED_LAYERS[key]] = features[key]
        return self._renamed_features


class TorchHubFeatureExtractor(HCNNFeatureExtractor):
    """
    ...
    def __init__(self, model_name, mode):
        ...
        assert model_name in [
            ...
            "resnet50",
            "vgg16",
        ], "The given Model is not managed by the current version of this Class."
        assert mode in [
            "pretrained",
            "untrained",
        ], "The version of the Model, pre/un-trained has to be specified."
        ...
        if mode == "pretrained":
            ...
            np.random.seed(
                0
            )  # ensure reproductibility of random weights initialisation !
            model = torch.hub.load(
        ...
        super(TorchHubFeatureExtractor, self).__init__(
            model, LAYERS[model_name], RENAMED_LAYERS
        )
